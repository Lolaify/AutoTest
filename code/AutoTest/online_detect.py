...
num_procs = max(1, min(int(n_cores), os.cpu_count()) // 4)
...
test_matching_dict = utils.build_matching_idx_dict_from_pre_list_parallel(df, pre_list, n_proc = num_procs, sbert_dist_val_embeddings = sbert_dist_val_embeddings, doduo_dist_val_scores = doduo_dist_val_scores)


results = []
if any([rule[1][0] == 'cta' for rule in rule_list]):
    sub_rule_list = [rule for rule in rule_list if rule[1][0] == 'cta']
    results += sherlock_check.sherlock_all_outliers_parallel(df, test_matching_dict, sub_rule_list, n_proc = num_procs)
if any([rule[1][0] == 'doduo' for rule in rule_list]):
    sub_rule_list = [rule for rule in rule_list if rule[1][0] == 'doduo']
    results += doduo_check.doduo_all_outliers_parallel(df, test_matching_dict, sub_rule_list, n_proc = num_procs, doduo_dist_val_scores = doduo_dist_val_scores)
if any([rule[1][0] == 'embed' for rule in rule_list]):
    sub_rule_list = [rule for rule in rule_list if rule[1][0] == 'embed']
    results += embed_check.embed_all_outliers_parallel(df, test_matching_dict, sub_rule_list, n_proc = num_procs)
if any([rule[1][0] == 'sbert' for rule in rule_list]):
    sub_rule_list = [rule for rule in rule_list if rule[1][0] == 'sbert']
    results += sbert_check.sbert_all_outliers_parallel(df, test_matching_dict, sub_rule_list, n_proc = num_procs, sbert_dist_val_embeddings = sbert_dist_val_embeddings)
